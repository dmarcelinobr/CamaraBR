...
buildRollcallDataset <- function (year = 2020, type = "", download = TRUE, ascii = FALSE, outfile = "cham_data_votes/data_") {
  ...
  if (download == TRUE) {
    .proposalDetails <- purrr::map_df(1989:year,~{loadCamaraProposals(.x)})
  }
...
.votacoesPlenarioVotos <- dplyr::left_join(.votacoesPlenarioVotos, .votacoesPlenarioCamara)

.data <- dplyr::left_join(.votacoesPlenarioVotos, .proposalDetails) %>% 
  dplyr::select(bill_id, rollcall_id, type_bill, number_bill, year_bill, decision_summary, decision_date, decision_time, rollcall_subject, rollcall_keywords, legislator_id, legislator_name, legislator_party, legislator_state, legislator_vote, sigla_orgao, orientation, GOV_orientation)
